    ...
    public class CartAPIController : ControllerBase
    {
        ...
        public async Task<ResponseDto> GetCart(string userId)
        {
            try
            {
                ...
                //Obtenemos los productos de la API de productos
                ...
                // aplicar coupon si existe
                if (!string.IsNullOrEmpty(cart.CartHeader.CouponCode))
                {
                    //Obtenemos el cupon de la API de Cupones
        ...
        public async Task<object> EmailCartRequest([FromBody] CartDto cartDto)
        {
            try
            {
                var emailShoppingCart = _configuration.GetValue<string>("TopicAndQueueNames:EmailShoppingCart");
        ...
        public async Task<ResponseDto> CartUpsert(CartDto cartDto)
        {
            try
            {
                // 1. Buscar el CartHeader del usuario (solo lectura)
                var cartHeaderFromDb = await _db.CartHeaders
                                                .AsNoTracking()
                                                .FirstOrDefaultAsync(u => u.UserId == cartDto.CartHeader.UserId);

                // CASO 1: El usuario NO tiene CartHeader → crear todo
                ...
                else
                {
                    ...
                    // CASO 2: Sí existe CartHeader → verificar detalle
